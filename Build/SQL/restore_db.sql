...
declare @restoredbpath varchar(255)
--Below is crude of finding default db path
set @restoredbpath=(SELECT SUBSTRING(physical_name, 1, CHARINDEX(N'master.mdf', LOWER(physical_name)) - 1)
                  FROM master.sys.master_files
                  WHERE database_id = 1 AND file_id = 1)
declare  @data_file_path varchar(255)
set  @data_file_path= @restoredbpath +  '$(dbname)' + '.mdf'
declare @log_file_path varchar(255)
set @log_file_path= @restoredbpath  + '$(dbname)' + '_log.ldf'
...
INSERT  @MyTempTable
    EXECUTE('RESTORE FILELISTONLY FROM DISK=''$(dbBackupPath)''')	
...
RESTORE DATABASE [$(dbname)] FROM DISK = '$(dbBackupPath)' WITH REPLACE,  MOVE  @data_LogicalName to @data_file_path, Move @log_logicalName to @log_file_path
GO
------------------------------------------------------------------------------